...
class handler(BaseHTTPRequestHandler):
    ...
    def scrape_netkeiba(self, url):
        ...
        # Meta Data (PC/SP/Title fallback)
        race_name_elem = (soup.select_one(".RaceName") or soup.select_one(".Race_Name") or 
        ...
        course_elem = soup.select_one(".RaceData01") or soup.select_one(".Race_Name_Box") or soup.select_one(".RaceData")
        course_info = ""
        if course_elem:
        ...
        if is_result:
            ...
            for i, row in enumerate(rows):
                try:
                    cols = row.select('td')
                    if len(cols) >= 10: # PC Layout
                        placing = cols[0].text.strip()
                        umaban = int(cols[2].text.strip())
                        name_a = cols[3].select_one('a')
                        name = name_a.text.strip() if name_a else cols[3].text.strip()
        ...
        for tbl in soup.select('.Payout_Detail_Table'):
            for tr in tbl.select('tr'):
                ...
                if tn in ['単勝', 'ワイド', '3連複']:
                    tds = tr.select('td')
                    if len(tds) >= 2: payouts[tn] = tds[1].get_text().strip().replace('\n', ' ')

        return {
            "race_name": race_name, "venue": venue, "race_num": race_num,
            "course_info": course_info, "grade_info": grade_info, "date_info": "",
